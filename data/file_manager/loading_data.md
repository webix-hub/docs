...
Dynamic loading means loading data on request, from the server. For example, a PHP script can supply data dynamically to File Manager.
Dynamic loading is useful for long datasets since data populates File Manager in smaller portions. Initially, data fills the view to some extent and later on it's dynamically loaded. 
...
Data that are loaded initially can contain a minimum number of folders, e.g. only first-level folders. 
Folders that should be loaded on demand must contain the **webix_branch** attribute. 
...
When the content of the folder should be dynamically loaded, FileManager sends a request to the script set in the "branch" parameter. 
The request contains several parameters:

- action - the name of the dynamic loading mode
- source - the id of the folder where data has been loaded
