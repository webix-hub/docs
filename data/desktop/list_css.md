List CSS 
=============

<img src="image_maps/list_css.png">

<table class="inline">

	<tr class="row0">
		<th class="col0 centeralign">  CSS class  </th><th class="col1 centeralign">  Element  </th>
	</tr>
		<tr class="row1">
		<td class="col0 leftalign"><strong>.webix_list</strong>  </td><td class="col1"> list view </td>
	</tr>
	<tr class="row2">
		<td class="col0 leftalign"><strong>.webix_list_item</strong>  </td><td class="col1"> an item of the list </td>
	</tr>
	<tr class="row3">
		<td class="col0 leftalign"><strong>.webix_list_item.webix_selected</strong>  </td><td class="col1"> the selected item of the list </td>
	</tr>
</table>

Read more about [List](desktop/list.md)
