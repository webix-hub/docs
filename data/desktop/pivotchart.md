...
of large quantities of data and the relationships between them.
...
<li>Configuration window (appeared when the user clicks the 'Settings' button):<br><br>
...
- Fields dragged or initially set in the **Values** block define which data will be loaded to the chart. Each data property in the **Values** block presents an individual graph.
...
value then the chart will display the total balance of all contries for each year. Only one data property can be specified in the **Group By** block. 
...
chart (nearby the 'Settings' button).
...
<br>To enable the scale, check a checkbox in the configuration window.
